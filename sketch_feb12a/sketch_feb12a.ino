...
void Timer1_action()
{
  // Устанавливаем 1 ждём интервал кратный 14 тактов (Предположительно 16 тактов микросекунда минус 2 на комманды)
  // в теории получаем в цикле while задержку кратную микросекунде 
  // digitalwrite очень медлененый минимальный импульс 5 мкс. 
  // интервалы имеют погрешность в меньшую сторону 1 мкс. например где-то 750 нс.
  PORTB |= 1 << 2;
